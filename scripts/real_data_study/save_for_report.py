...
def save_tables_and_figures(run_id: int, save_to_git: bool):
    ...
    feature_header = [
        "& \\rotatebox{45}{\\texttt{" + feature + "}}"
        for feature in cont_features + cat_features
    ]
    kappa_table += " ".join(feature_header) + " \\\\ \n"

    kappa_row = "$\\kappa_j$ & "
    cont_features_kappa = " & ".join(
        [str(parameters.loc[feature, "n_estimators"]) for feature in cont_features]
    )
    kappa_row += cont_features_kappa
    for cat_feature in cat_features:
        dummy_features = [
            feature for feature in features if feature.startswith(cat_feature)
        ]
    ...
    beta_row = "$\\beta_{j0}$ & "
    cont_features_beta = " & ".join(
        [
            str(np.round(parameters.loc[feature, "beta0"], 2))
    ...
    dev_table = pd.DataFrame(
        columns=["Train", "Test"],
        index=["Intercept", "GLM", "LocalGLMBoost", "LocalGLMNet"],
        dtype=float,
    )

    dev_table.loc["Intercept", "Train"] = deviance(
        train_data["y"], train_data["w"], train_data["z_0"]
    )
    dev_table.loc["Intercept", "Test"] = deviance(
        test_data["y"], test_data["w"], test_data["z_0"]
    )
    dev_table.loc["GLM", "Train"] = deviance(
        train_data["y"], train_data["w"], train_data["z_glm"]
    )
    dev_table.loc["GLM", "Test"] = deviance(
        test_data["y"], test_data["w"], test_data["z_glm"]
    )
    dev_table.loc["LocalGLMBoost", "Train"] = deviance(
        train_data["y"], train_data["w"], train_data["z_local_glm_boost"]
    )
    dev_table.loc["LocalGLMBoost", "Test"] = deviance(
        test_data["y"], test_data["w"], test_data["z_local_glm_boost"]
    )
    ...
    window_size = 1000
    y_hat_smooth = (
        y_hat.sort_values("LocalGLMBoost")
    ...
    feature_importances = pd.read_csv(
        folder_path + "feature_importances.csv", index_col=0
    )

    cat_features = ["VehGas", "VehBrand", "Region"]
    for cat_feature in cat_features:
        dummy_features = [
            col for col in feature_importances.index if cat_feature in col
        ]
        feature_importances[cat_feature] = feature_importances[dummy_features].sum(
            axis=1
        )
        feature_importances.loc[cat_feature] = feature_importances.loc[
            dummy_features
        ].sum(axis=0)
    ...
    feature_importances = feature_importances.div(
        feature_importances.sum(axis=1), axis=0
    ).fillna(0)
    ...
    yticklabels = ", ".join(
        [f"\\texttt{{{col}}}" for col in feature_importances.columns]
    )
